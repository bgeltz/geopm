...
namespace geopm
{
    ...
    ConstConfigIOGroup::ConstConfigIOGroup()
        : ConstConfigIOGroup(M_DEFAULT_CONFIG_FILE_PATH, geopm::get_env(M_CONFIG_PATH_ENV))
    {
    }

    ConstConfigIOGroup::ConstConfigIOGroup(const std::string &default_file_path,
                                           const std::string &user_file_path)
    {
        ...
        if (!user_file_path.empty()) {
            ...
            // TODO: Catch the specific exception you expect: const geopm::Exception &ex
            catch (...) {
#ifdef GEOPM_DEBUG
