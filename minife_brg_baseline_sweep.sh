...
# Do the runs
#   Sweep the frequencies with wrmsr
LOOPS=1
for iter in $(seq 1 ${LOOPS}); do
    START=12
    STOP=13
    STEP=1
    for freq in $(seq ${START} ${STEP} ${STOP}); do
        init
        printf '%.0s*' {1..80} && echo
        echo Requesting $(( ${freq} * 100 )) MHz - 0x$(echo "obase=16; ${freq}" | bc)00
        printf '%.0s*' {1..80} && echo
        freq_hex=$(echo "obase=16; ${freq}" | bc)
        pdsh -w ${NODE_LIST} "${HOME}/build/msr-tools/bin/wrmsr -a 0x199 0x${freq_hex}00"
        # The frequency parameters do not matter for the baseline.  I'm only doing this so that the
        # profile names is set properly.
        run_app baseline ${NX} ${NY} ${NZ} ${iter} $((${freq} * 100000000))
        cleanup
    done
done
