...
static int main_imp(int argc, char **argv)
{
    ...
    char output_str[GEOPMAGENT_POLICY_MAX * GEOPMAGENT_DOUBLE_LENGTH];
                ...
                while (!err && tok != NULL) {
                    ...
                    if (tok == endptr) {
                        ...
                        if (policy_name.find("HASH") != std::string::npos ||
                            ...
                            policy_vals[policy_count] = static_cast<double>(geopm_crc32_str(tok));
                        }
                        else {
